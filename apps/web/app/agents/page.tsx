...
export default function AgentsPage() {
    ...
    const OPERATOR_METRICS = [
        { label: t.agents.metric_signals, value: "1,847", icon: Zap, color: "text-neural-cyan", sublabel: t.agents.metric_signals_sub, glow: "var(--color-neural-cyan)" },
        { label: t.agents.metric_approval, value: "98.2%", icon: Shield, color: "text-neural-green", sublabel: t.agents.metric_approval_sub, glow: "var(--color-neural-green)" },
        { label: t.agents.metric_uptime, value: "99.9%", icon: Activity, color: "text-neural-purple", sublabel: t.agents.metric_uptime_sub, glow: "var(--color-neural-purple)" },
        { label: t.agents.metric_ipfs, value: "QmXr7Y...", icon: Database, color: "text-neural-gold", sublabel: t.agents.metric_ipfs_sub, glow: "var(--color-neural-gold)" },
    ];

    const ACTIVE_ASSIGNMENTS = [
        { pool: "WETH/USDC", fee: "7200 bps", lastSignal: `12 ${t.home.how_title.includes('秒') ? '秒前' : t.home.how_title.includes('seconds') ? 'secs ago' : 'segs atrás'}`, status: "RUNNING" },
        { pool: "cbBTC/USDC", fee: "3000 bps", lastSignal: `8 ${t.home.how_title.includes('秒') ? '秒前' : t.home.how_title.includes('seconds') ? 'secs ago' : 'segs atrás'}`, status: "RUNNING" },
        { pool: "UNI/ETH", fee: "10000 bps", lastSignal: `3 ${t.home.how_title.includes('秒') ? '秒前' : t.home.how_title.includes('seconds') ? 'secs ago' : 'segs atrás'}`, status: "ALERT" },
    ];
        ...
        return (
            <div className="pt-32 flex flex-col items-center justify-center min-h-[70vh] gap-5">
                <div className="p-6 rounded-2xl bg-white/3 border border-white/8">
                    ...
                    <h2 className="text-xl font-bold text-center text-white mb-2">{t.agents.connect_title}</h2>
                    <p className="text-gray-500 text-sm text-center max-w-xs">{t.agents.connect_desc}</p>
                </div>
            </div>
        );
    }
                ...
                <div className="pt-8 mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <div className="inline-flex items-center gap-2 neon-badge mb-4">
                            ...
                            {t.agents.node_online}
                        </div>
                        <h1 className="text-5xl font-black tracking-tighter mb-2">
                            {t.agents.console_title.split(' ')[0]} {" "}
                            <span className="text-transparent bg-clip-text bg-linear-to-r from-neural-magenta to-neural-cyan">
                                {t.agents.console_title.split(' ')[1] || 'Console'}
                            </span>
                        </h1>
                        <p className="text-gray-500">
                            {t.agents.console_desc}{" "}
                    ...
                    <div className="flex gap-2 shrink-0">
                        ...
                        >
                            <Settings size={15} /> {t.agents.node_settings}
                        </motion.button>
                        <motion.button
                            whileHover={{ scale: 1.03 }}
                            whileTap={{ scale: 0.97 }}
                            onClick={() => toast.success(t.agents.keystore_success)}
                            className="btn-ghost flex items-center gap-2 px-4 py-2.5 text-sm"
                        >
                            <Download size={15} /> {t.agents.download_keystore}
                        </motion.button>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 mb-8">
                    ...
                    <div className="lg:col-span-1 glass-card p-5">
                        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-400 font-mono mb-4 flex items-center gap-2">
                            <Server size={14} className="text-neural-magenta" /> {t.agents.active_assignments}
                        </h2>
                                ...
                                >
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className={a.status === "RUNNING" ? "status-running" : "px-2 py-0.5 rounded text-xs font-mono font-bold text-neural-red bg-neural-red/10 border border-neural-red/30 uppercase animate-pulse"}>
                                            {a.status === "RUNNING" ? t.agents.status_running : t.agents.status_alert}
                                        </span>
                                    </div>
                                </div>
                            ))}
                    ...
                    <div className="lg:col-span-2 glass-card p-5 overflow-x-auto">
                        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-400 font-mono mb-4 flex items-center gap-2">
                            <Zap size={14} className="text-neural-cyan" /> {t.agents.recent_submissions}
                        </h2>
                        <table className="w-full text-xs font-mono">
                            <thead>
                                <tr className="text-gray-600 border-b border-white/5">
                                    {[t.agents.table_block, t.agents.table_pool, t.agents.table_fee, t.agents.table_action, t.agents.table_cid, t.agents.table_status].map((h) => (
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td className="pr-4 text-gray-400">
                                                {s.action === "FEE_ADJUST" ? t.agents.action_fee_adjust : t.agents.action_il_activate}
                                            </td>
                                            <td className="pr-4">
                                            ...
                                            <td className={rejected ? "text-neural-red" : "text-neural-green"}>
                                                {s.status === "APPROVED" ? t.agents.status_approved : t.agents.status_rejected}
                                            </td>
                                        </motion.tr>
                                    );
                                })}
                ...
                <div className="glass-card p-6">
                    <div className="flex items-center justify-between mb-6">
                        <h2 className="text-sm font-bold uppercase tracking-wider text-gray-400 font-mono flex items-center gap-2">
                            <Trophy size={14} className="text-neural-gold" /> {t.agents.network_leaderboard}
                        </h2>
                        <Link href="/leaderboard" className="text-xs text-neural-magenta hover:underline flex items-center gap-1 font-mono uppercase tracking-tighter">
                            {t.agents.full_rankings} <Activity size={10} />
                        </Link>
                    </div>
                        ...
                        {LEADERBOARD_DATA.slice(0, 5).map((entry, i) => (
                            ...
                            >
                                ...
                                <div className="text-right shrink-0">
                                    ...
                                    <div className="text-[9px] text-white/20">{t.agents.signals}</div>
                                </div>
                                <div className="text-right shrink-0 hidden sm:block">
                                    <div className="text-sm font-bold font-mono text-neural-cyan">
                                        {entry.accuracy}%
                                    </div>
                                    <div className="text-[9px] text-white/20">{t.agents.accuracy}</div>
                                </div>
                            </motion.div>
                        ))}
                    </div>
